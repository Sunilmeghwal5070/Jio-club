...
import { LayoutGrid, QrCode, Copy } from 'lucide-react';
...
export function DepositHistory() {
  ...
  const types = ['All', 'UPI-QR', 'UPI-QR PAY', 'PAYTM'];
      ...
      <div className="px-4">
        ...
        <div className="flex gap-2 overflow-x-auto py-4 scrollbar-hide">
           {types.map(type => (
             ...
             >
               {type === 'All' ? <LayoutGrid size={16} /> : <QrCode size={16} />}
               {type}
             </button>
           ))}
        </div>
           ...
           {filtered.length > 0 ? filtered.map(tx => (
             <div key={tx.id} className="bg-card-base rounded-xl p-4 shadow-sm border border-card-base">
                <div className="flex justify-between items-center mb-4">
                  <div className="bg-green-500 text-white text-sm font-bold px-3 py-1 rounded">Deposit</div>
                ...
                <div className="flex flex-col gap-3">
                   <div className="flex justify-between text-sm">
                     ...
                     <span className="text-primary font-medium">₹{tx.amount.toFixed(2)}</span>
                   </div>
                   <div className="flex justify-between text-sm">
